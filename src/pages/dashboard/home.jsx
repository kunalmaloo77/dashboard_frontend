...
export function Home() {
  ...
  const getData = async (startDate, endDate, type) => {
    try {
      ...
      const temp = Object.values(
        res.data.reduce((result, currentValue) => {
  ...
  useEffect(() => {
    ...
    const sevenDaysFromToday = new Date(2024, 5, 1); // change it to today if data is entered
    sevenDaysFromToday.setDate(today.getDate() - 7);

    const fifteenDaysBeforeSevenDaysFromToday = new Date(sevenDaysFromToday);
    fifteenDaysBeforeSevenDaysFromToday.setDate(
      sevenDaysFromToday.getDate() - 15,
    );
  ...
  return (
    <div className="mt-12">
      <div className="mb-6">
        <Card className="mb-6 w-full max-w-[40rem]">
                  ...
                  footer={
                    <div className="flex gap-2">
                      <Typography variant="h6" className="flex items-center">
                        ...
                        {((delivered / (delivered + returned)) * 100).toFixed(
                          2,
                        )}{" "}
                        %
                      </Typography>
                      <Typography variant="h6" className="flex items-center">
                        Return percentage ={" "}
                        {((returned / (delivered + returned)) * 100).toFixed(2)}{" "}
                        %
                      </Typography>
                    </div>
                  }
