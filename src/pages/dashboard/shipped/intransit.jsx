...
const Intransit = () => {
  ...
  const getShipped = async () => {
    try {
      ...
      const shipped = res.data.shipped.map(order => {
      ...
      setLoading(false);
      console.log(error, "<-intransit error");
    }
  ...
  return (
    <div>
      <div className='inline-block'>
        {allData.length > 0 ? (
          <CSVLink data={allData} headers={headers} filename={"intransit.csv"}>
          ...
          <Button onClick={downloadIntransitOrders} className="mt-2 flex items-center justify-center gap-2 ml-4" color='green' disabled={loading1 ? true : false}>
            {loading1 ?
              <div role="status">
